...
static uint64_t alloc_node(Brain *g) {
    if (g->header->num_nodes >= g->header->node_capacity) return UINT64_MAX;
...
static void add_edge(Brain *g, uint64_t src, uint64_t dst, float w, uint32_t flags) {
    if (g->header->num_edges >= g->header->edge_capacity) return;
...
static uint64_t create_string_node(Brain *g, const char *text, size_t len) {
    ...
    uint64_t node_id = find_or_create_node(g, hash, NODE_KIND_DATA);
    
    if (node_id != UINT64_MAX && node_id != 0) {
