#!/bin/bash
# Initialize melvin.m brain file with dynamic size based on available disk space
# The limit should be disk space, not an arbitrary number

set -e

echo "=========================================="
echo "Initializing Melvin Brain (Dynamic Size)"
echo "=========================================="
...
echo "Usable space (90%): $(numfmt --to=iec-i --suffix=B $USABLE_SPACE)"
echo ""

# Node and Edge sizes (from melvin.h)
...
# Formula: HEADER + NODE_CAP * NODE_SIZE + EDGE_CAP * EDGE_SIZE = USABLE_SPACE
...
echo "Brain file parameters:"
echo "  Node capacity: $NODE_CAP ($(numfmt --to=iec-i --suffix=B $((NODE_CAP * NODE_SIZE))))"
echo "  Edge capacity: $EDGE_CAP ($(numfmt --to=iec-i --suffix=B $((EDGE_CAP * EDGE_SIZE))))"
echo "  Total file size: $(numfmt --to=iec-i --suffix=B $TOTAL_SIZE)"
echo ""

# Create initialization program (same as before, but uses calculated values)
...
echo "Capacity limited only by disk space: $(numfmt --to=iec-i --suffix=B $USABLE_SPACE)"
echo "Ready to run: ./melvin $BRAIN_FILE"
echo ""
