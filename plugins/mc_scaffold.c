...
static uint64_t ensure_blank_node(Brain *g, const char *var_name) {
    ...
    // Search for existing blank with same hash
    uint64_t n = g->header->num_nodes;
    for (uint64_t i = 0; i < n; i++) {
        Node *node = &g->nodes[i];
        if (node->kind == NODE_KIND_BLANK && (uint32_t)node->value == hash) {
            return i;
        }
    }
    
    // Create new blank node
    uint64_t blank_node = alloc_node(g);
    if (blank_node == UINT64_MAX) return UINT64_MAX;
    
    Node *blank = &g->nodes[blank_node];
    blank->kind = NODE_KIND_BLANK;
    blank->a = 0.3f;
    blank->value = (float)hash;
    blank->flags = 0;
...
void mc_scaffold_emit_rule(Brain *g, const ScannedRule *r) {
    ...
    printf("[mc_scaffold] Pattern node %llu created for rule %s\n", 
           (unsigned long long)pattern_node, r->name);
    ...
    printf("[mc_scaffold] Rule %s fully emitted to graph\n", r->name);
}
